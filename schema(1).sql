...
create or replace function check_validity_meio_alocado()
...
begin
  ...
  if not exists (
    select *
    from acciona a
    where a.numProcessoSocorro = new.numProcessoSocorro and 
          a.numMeio = new.numMeio and a.nomeEntidade = new.nomeEntidade )
  then
    ...
    using hint = 'So se pode alocar meios de apoio a processos de socorro para os quais tenha sido accionado.';
  end if;
...
create or replace function check_validity_coordenador_solicita()
returns trigger as $body$
begin

  if not exists(
    select *
    from (audita natural join eventoEmergencia natural join vigia) s
    where s.idCoordenador = new.idCoordenador and s.numCamara = new.numCamara
  )
  then
    raise exception 'Os videos da camara % nao podem ser solicitados pelo cooordenador %.', new.numCamara, new.idCoordenador
    using hint = 'Um coordenador so pode solicitar videos de camaras colocadas num local cujo acionamento de meios esteja a ser (ou tenha sido) auditado por ele proprio.';
  end if;
